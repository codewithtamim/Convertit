...
object CueParser {
    ...
    private fun parseCueLines(lines: List<String>): CueFile {
        ...
        while (i < lines.size) {
            ...
            if (trimmedLine.isEmpty() || trimmedLine.startsWith("REM", ignoreCase = true)) {
                i++
                continue
            }
            ...
            when {
                ...
                trimmedLine.startsWith("TRACK", ignoreCase = true) -> {
                    ...
                    val trackType = extractTrackType(trimmedLine)
                    
                    // Only process audio tracks
                    if (trackNumber != null && (trackType == null || trackType.equals("AUDIO", ignoreCase = true))) {
                ...
                trimmedLine.startsWith("INDEX 01", ignoreCase = true) -> {
                    ...
                    if (currentTrackNumber != null && timeStamp != null) {
                        val timeSeconds = convertTimeToSeconds(timeStamp)
                        if (timeSeconds >= 0) { // Only add valid timestamps
                            val track = CueTrack(
    ...
    private fun extractQuotedValue(line: String): String? {
        ...
        val quotedMatch = quotedRegex.find(line)
        if (quotedMatch != null) {
            return quotedMatch.groupValues[1].replace("\\\"", "\"")
        }
    ...
    private fun extractTrackType(line: String): String? {
        val parts = line.split("\\s+".toRegex())
    ...
    private fun convertTimeToSeconds(timeStamp: String): Double {
        ...
        return if (parts.size == 3) {
            ...
            if (minutes < 0 || seconds < 0 || frames < 0 || 
                ...
                Log.w(TAG, "Invalid time stamp: $timeStamp")
                return 0.0
            }
            
            minutes * 60 + seconds + (frames / 75.0)
        } else {
            Log.w(TAG, "Invalid time stamp format: $timeStamp")
            0.0
        }
    }
