...
class IntentLauncher(
    ...
    fun openAudioFromLibrary(audioFile: AudioFile) {
        audioFile.contentUri?.let { uri ->
            val intent =
    ...
    fun shareAudioFromLibrary(audioFile: AudioFile) {
        audioFile.contentUri?.let { uri ->
            val shareIntent =
    ...
    fun shareMultipleAudioFiles(audioFiles: List<AudioFile>) {
        val uris = ArrayList<Uri>()
        audioFiles.forEach { audioFile ->
            audioFile.contentUri?.let { uris.add(it) }
                ?: audioFile.file?.takeIf { it.exists() }?.let { file ->
                    uris.add(
